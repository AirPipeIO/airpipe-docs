...
You can use the [assertion](/docs/reference/asserts) system to validate prior actions, and then also use special keywords to prevent the next actions from executing conditionally.
        ...
        assert:  # use the assert/test system to validate the payload
          tests:
            - value: customer # target the customer attribute from a|body| payload
              ...
              data_type: String # compare how many characters, not the value
              is_greater_than: 2
              is_less_than: 20
