...
| ------------- | ------ | --------------------------------- |
| `driver`      | string | the database driver i.e. postgres |
| `conn_string` | string | the database connection string    |
...
Run configuration files on a cron like interval.
...
global:
  scheduler:
    get-post:
      rate: 1/10 * * * * *
      http:
        url: http://0.0.0.0:4111/tests/get-headers
        headers:
          customheader: hello
```
