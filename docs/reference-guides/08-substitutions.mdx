...
To substitute variables the following convention is followed `a|<something>|`.
    ...
    - The same closed `a|…|` form is used in the `input:` parameter to select a data source, e.g. `input: a|body|`, `input: a|params|`, `input: a|headers|`. (Older docs showed these without the trailing `|`; that open form is deprecated — always close the marker.)
