...
The `discover` action returns the live members of a service as an array, and
`lookup:` fans actions out across them — so you stop hard-coding a list of
addresses that is wrong the moment anything scales.
...
Four backends, one item shape — `{ name, address, port, url, source, ready,
namespace, node, image, labels }` — so a config moves between orchestrators by
changing only the backend block:

| Backend | Targeting | What it costs |
|---|---|---|
| `dns` | none — addresses only | **nothing**: no API access, no credentials, no RBAC |
| `kubernetes` | label + field selectors, readiness, node, image | a ServiceAccount with `list` on pods |
| `docker` | container labels, image | the Docker socket (unix only) |
| `static` | — | nothing; an explicit list |

**Reach for `dns` first.** It finds the same pods as the Kubernetes backend and
needs no privilege at all. Use the API backend when you actually need label
selectors or pod metadata:
...
That needs a namespace-scoped Role:
...
namespace**, which keeps each grant visible where it applies.

Two options worth setting deliberately:

- **`cache_ttl_secs`** (default 10). Discovery on a request-path interface would
  otherwise issue an API call per request, turning a busy route into a
  denial-of-service against your own API server.
- **`fail_when_empty`** (default false). An empty result is an empty loop,
  because a service scaled to zero is not an error.

Fields a backend cannot know are **omitted, not defaulted**: DNS has no image, so
an `image` filter matches nothing there rather than silently passing.
`include`/`exclude` take regexes, and **exclusion beats inclusion**.
