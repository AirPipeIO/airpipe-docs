      ...
      containers:
        - name: airpipe
          ...
          env:
            ...
            # The engine's database: scheduler coordination, run history and
            # durable state. One URL covers all three.
...
line. Coordination happens through Postgres: with `AIRPIPE__DATABASE_URL`
...
counters. It uses the same `AIRPIPE__DATABASE_URL` as above, so a clustered
deployment that has named its database already has durable state. (Set
`AIRPIPE__STATE_DATABASE_URL` only to keep state in a *different* database.)
