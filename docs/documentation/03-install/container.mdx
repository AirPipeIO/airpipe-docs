...
There is a published multi-architecture image, so most deployments need no
Dockerfile at all:
...
Running more than one replica? See [Kubernetes](/docs/install/kubernetes) — scheduled
jobs, realtime fan-out and state all behave differently once there are peers, and
the same settings apply to Docker Compose.

## Build your own image

Prefer to control the base image, add your own tooling, or bake configs in? The
rest of this page builds an image from scratch.
